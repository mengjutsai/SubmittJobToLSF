...
########   customization  area #########
MaxInputPerJob = 20 # number files to be processed in a single job, take care to split your file so that you run on all files. The last job might be with smaller number of files (the ones that remain).
        ...
        for run_number in content_no_replica:
            ...
            if interval == 0:
                ...
                if isfile("log.txt"):
                    ...
                    if not check:
                        print("List is empty")
                        Run_Pend_Fail_Job = Run_Pend_Fail_Job + 1
                    else:
                        if 'code' in check[0] and '"successful' in check[0] and 'run"' in check[0]:
                            print("YES")
                            FinishJob = FinishJob +1
                        else:
                            Run_Pend_Fail_Job = Run_Pend_Fail_Job + 1
                            print("Please Rusubmit the jobs!")
                            if ReSubmiited:
...
print "FinishJob ", FinishJob, ", Run_Pend_Fail_Job ", Run_Pend_Fail_Job
os.system("bjobs")
print
print 'END'
print
